...
URL.BaseAppUrl = URL.RootAppUrl + "/" + window.location.pathname.split("/")[2] + "/" + window.location.pathname.split("/")[3];
URL.RouterBaseAppUrl = "/router/" + window.location.pathname.split("/")[2] + "/" + window.location.pathname.split("/")[3];
...
URL.LogUrl = URL.BaseAppUrl + "/log"; // URL.versionNumber = versionNumberMeta;
...
utils.makeAjaxJSONCall = function (uri, type, jsonObject, callback, dataServiceName, forwardedData, retryCount) {
  myModule.fetch({
    url: uri,
    type: type,
    startTime: Date.now(),
...
utils.sendErrorToServer = function (error) {
  ...
  utils.makeAjaxJSONCall(URL.LogUrl + "/error", ReqType.POST, error, function () {
...
utils.sendInfoToServer = function (info) {
  ...
  utils.makeAjaxJSONCall(URL.LogUrl + "/info", ReqType.POST, jsonObj, function () {
    /* No Need to do anything */
  }, null, null);
};
...
utils.sendDebugToServer = function (debug) {
  ...
  utils.makeAjaxJSONCall(URL.LogUrl + "/debug", ReqType.POST, jsonObj, function () {
    /* No Need to do anything */
  }, null, null);
};
